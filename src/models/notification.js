...
const notificationSchema = new Schema({
  ...
  recipients: [{
    userId: { type: Schema.Types.ObjectId, ref: 'User', required: true },
    userRole: { type: String, enum: ['customer', 'admin', 'vendor', 'support', 'manager'] },
    personalizedData: { type: Map, of: Schema.Types.Mixed }
  }],
  targetAudience: {
...
notificationSchema.virtual('hasMedia').get(function() {
  return !!(this.media &&
            (this.media.imageUrl || (Array.isArray(this.media.attachments) && this.media.attachments.length > 0)));
});
...
notificationSchema.virtual('displayTitle').get(function() {
  if (this.localization &&
      ...
      this.localization.translations &&
      this.localization.translations[this.localization.language] &&
      this.localization.translations[this.localization.language].title) {
...
notificationSchema.virtual('displayMessage').get(function() {
  if (this.localization &&
      ...
      this.localization.translations &&
      this.localization.translations[this.localization.language] &&
      this.localization.translations[this.localization.language].message) {
...
notificationSchema.virtual('lastReadAt').get(function() {
  ...
  return this.readBy.reduce((latest, r) => 
    !latest || r.readAt > latest ? r.readAt : latest, null
  );
});
...
notificationSchema.virtual('unreadUserIds').get(function() {
  ...
  return this.recipients
    .filter(rec => !readIds.has(rec.userId.toString()))
    .map(rec => rec.userId);
});
...
notificationSchema.virtual('hasClickableActions').get(function() {
  return Array.isArray(this.actions) &&
    this.actions.some(act =>
      ['redirect', 'api_call', 'modal'].includes(act.action)
    );
});
...
notificationSchema.virtual('hasAttachments').get(function() {
  return !!(this.media &&
    Array.isArray(this.media.attachments) &&
    this.media.attachments.length > 0
  );
});
...
notificationSchema.post('save', async function(doc, next) {
  try {
    ...
    if (doc.realTimeConfig?.enableRealTime && typeof require !== 'undefined') {
      ...
      } catch (e) { /* ignore socket errors to not fail save */ }
    }
...
notificationSchema.methods.getDeliveryReport = function() {
  ...
  return {
    ...
    createdAt: this.createdAt,
    lastActivity: this.readBy.length > 0
...
notificationSchema.methods.batchMarkAsRead = async function(userIds) {
  userIds.forEach(userId => {
    if (!this.readBy.some(r => r.userId.toString() === userId.toString()) &&
        this.recipients.some(r => r.userId.toString() === userId.toString())) {
...
notificationSchema.statics.findForUser = function(userId, options = {}) {
  const {
    status, type, priority, limit = 50, skip = 0,
    sortBy = 'createdAt', sortOrder = -1, includeArchived = false,
  } = options;
...
notificationSchema.statics.cleanupExpired = async function() {
  return this.updateMany(
    ...
    { $set: { isArchived: true, archivedAt: new Date(), status: 'archived' } }
  );
};
