...
resumeSchema.statics.getSingleWithProjectCount = async function (id, options = {}) {
    ...
    if (ObjectId.isValid(id)) {
        ...
        filter = {
            ...
            $or: [
                { _id: id },
                { user: id, isDefault: true, currentlyUsed: true }
            ],
        };
    } else {
...
resumeSchema.statics.getListWithProjectCounts = async function (
  filter = {},
  options = {}
) {
  ...
  const facetsPipeline = facetFields.length
    ? [{
        $facet: facetFields.reduce((acc, field) => {
          acc[field] = [
            { $unwind: { path: `$${field.split('.')[0]}`, preserveNullAndEmptyArrays: true } },
