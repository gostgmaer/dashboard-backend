...
brandSchema.statics.searchBrands = function (keyword) {
  return this.find({
    $or: [{ name: { $regex: keyword, $options: 'i' } }, { descriptions: { $regex: keyword, $options: 'i' } }, { 'seo.keywords': { $regex: keyword, $options: 'i' } }],
  }).sort({ name: 1 });
};

// Bulk update status
brandSchema.statics.bulkUpdateStatus = function (ids, status) {
  return this.updateMany({ _id: { $in: ids } }, { $set: { status, updatedAt: new Date() } }, { runValidators: true });
};
...
brandSchema.statics.getTopRatedBrands = function (limit = 10) {
  return this.find({ isActive: true }).sort({ rating: -1, name: 1 }).limit(limit);
};
...
brandSchema.statics.getPaginatedBrands = async function ({ page = 1, limit = 10, status, search, sortBy = 'name', sortOrder = 1 }) {
  ...
  if (search) {
    query.$or = [{ name: { $regex: search, $options: 'i' } }, { descriptions: { $regex: search, $options: 'i' } }, { 'seo.keywords': { $regex: search, $options: 'i' } }];
  }

  const skip = (page - 1) * limit;
  const sort = { [sortBy]: sortOrder };

  const [items, total] = await Promise.all([this.find(query).sort(sort).skip(skip).limit(limit), this.countDocuments(query)]);
...
brandSchema.statics.bulkFeatureToggle = function (ids, isFeatured) {
  return this.updateMany({ _id: { $in: ids } }, { $set: { isFeatured, updatedAt: new Date() } }, { runValidators: true });
};

// Soft delete brands (mark inactive instead of removing)
brandSchema.statics.softDeleteBrands = function (ids) {
  return this.updateMany({ _id: { $in: ids } }, { $set: { isActive: false, status: 'inactive', updatedAt: new Date() } }, { runValidators: true });
};

// Restore soft-deleted brands
brandSchema.statics.restoreBrands = function (ids) {
  return this.updateMany({ _id: { $in: ids } }, { $set: { isActive: true, status: 'active', updatedAt: new Date() } }, { runValidators: true });
};
...
brandSchema.statics.getBrandsWithSocialMedia = function () {
  return this.find({
    $or: [{ 'socialMedia.facebook': { $ne: '' } }, { 'socialMedia.twitter': { $ne: '' } }, { 'socialMedia.instagram': { $ne: '' } }, { 'socialMedia.linkedin': { $ne: '' } }],
    isActive: true,
    status: 'active',
  }).sort({ name: 1 });
};
