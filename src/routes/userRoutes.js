...
const userValidation = {
  ...
  bulkUpdate: [body('ids').isArray({ min: 1 }).withMessage('User IDs array is required'), body('ids.*').isMongoId().withMessage('Invalid user ID in array'), body('updateData').optional().isObject().withMessage('Update data must be an object'), body('status').optional().isIn(['active', 'inactive', 'suspended', 'pending']).withMessage('Invalid status'), validate],

  wishlist: [param('id').isMongoId().withMessage('Invalid user ID'), body('productId').isMongoId().withMessage('Valid product ID is required'), validate],

  cart: [param('id').isMongoId().withMessage('Invalid user ID'), body('productId').isMongoId().withMessage('Valid product ID is required'), body('quantity').optional().isInt({ min: 1 }).withMessage('Quantity must be a positive integer').toInt(), validate],

  loyalty: [param('id').isMongoId().withMessage('Invalid user ID'), body('points').isInt({ min: 1 }).withMessage('Points must be a positive integer').toInt(), validate],

  address: [param('id').isMongoId().withMessage('Invalid user ID'), body('address').isObject().withMessage('Address must be an object'), body('address.street').notEmpty().withMessage('Street is required').trim().escape(), body('address.city').notEmpty().withMessage('City is required').trim().escape(), body('address.country').notEmpty().withMessage('Country is required').trim().escape(), body('address.zipCode').notEmpty().withMessage('ZIP code is required').trim().escape(), validate],

  search: [query('keyword').optional().notEmpty().withMessage('Search keyword is required').trim().escape(), query('limit').optional().isInt({ min: 1, max: 100 }).withMessage('Limit must be between 1 and 100').toInt(), query('status').optional().isIn(['active', 'inactive', 'suspended', 'pending']).withMessage('Invalid status filter'), validate],

  advancedSearch: [query('criteria').optional().isObject().withMessage('Criteria must be an object'), query('limit').optional().isInt({ min: 1, max: 100 }).withMessage('Limit must be between 1 and 100').toInt(), validate],

  notify: [param('id').isMongoId().withMessage('Invalid user ID'), body('message').notEmpty().withMessage('Message is required').trim().escape(), validate],

  exportImport: [query('format').optional().isIn(['csv', 'json']).withMessage('Invalid format'), validate],
};
...
router.get('/profile', authMiddleware, UserController.getMyProfileStatisticsController);
...
router.delete('/:id', authMiddleware, authorize('users', 'delete'), instanceCheckMiddleware, param('id').isMongoId().withMessage('Invalid user ID'), validate, UserController.deleteUser);
...
router.post(
  '/:id/favorites/move-from-wishlist',
  authMiddleware,
...
router.get(
  '/analytics/count-by-subscription',
  authMiddleware,
...
router.get(
  '/analytics/average-loyalty-points',
  authMiddleware,
...
router.get(
  '/analytics/login-activity-over-time',
  authMiddleware,
...
router.get(
  '/analytics/activity-summary/:userId',
  authMiddleware,
...
router.get(
  '/analytics/users-with-analytics',
  authMiddleware,
...
const routeOrderMiddleware = (req, res, next) => {
  ...
  if (path.startsWith('/authentication/') || path.startsWith('/search/') || path.startsWith('/filter/') || path.startsWith('/analytics/') || path.startsWith('/bulk/') || path.startsWith('/export/') || path.startsWith('/import/') || path === '/advanced-search' || path === '/profile') {
    return next();
  }

  const segments = path.split('/').filter(Boolean);
  if (segments.length > 1) {
    const specificPatterns = ['profile', 'wishlist', 'favorites', 'cart', 'preferences', 'loyalty', 'subscription', 'status', 'addresses', 'payment-methods', 'social-media', 'interests', 'sessions', 'orders', 'statistics', 'report', 'activity-summary', 'dynamic-update', 'notify'];
...
router.get(
  ...
  (req, res) => {
    ...
    const routes = {
      crud: ['POST   /api/users                             - Create new user (write)', ' POST /api/users/register - Register new user Write', 'GET    /api/users                             - Get all users with filtering (read)', 'GET    /api/users/:id                         - Get single user by ID (read, instance check)', 'PUT    /api/users/:id                         - Update user (update, instance check)', 'PATCH    /api/users/:userId/role                         - Assign Role (update, instance check)', 'DELETE /api/users/:id                         - Delete user (soft delete) (update, instance check)'],
      authentication: ['POST   /api/users/authentication/login                  - User login (no auth)', 'PUT    /api/users/:id/authentication/change-password    - Change password (update, instance check)', 'POST   /api/users/authentication/reset-token            - Generate reset token (write)', 'POST   /api/users/authentication/reset-password         - Reset password with token (update)', 'POST   /api/users/authentication/confirm-email          - Confirm email (update)', 'PUT    /api/users/:id/authentication/verify             - Verify user (update, instance check)'],
      profile: ['PUT    /api/users/:id/profile                 - Update profile (update, instance check)', 'GET    /api/users/profile                     - Get profile (read)', 'PUT    /api/users/:id/profile/picture         - Update profile picture (update, instance check)', 'PUT    /api/users/:id/profile/email           - Update email (update, instance check)', 'PUT    /api/users/:id/profile/phone           - Update phone number (update, instance check)'],
      wishlist: ['POST   /api/users/:id/wishlist                - Add to wishlist (write, instance check)', 'DELETE /api/users/:id/wishlist                - Remove from wishlist (update, instance check)', 'DELETE /api/users/:id/wishlist/clear          - Clear wishlist (update, instance check)', 'GET    /api/users/:id/wishlist/count          - Get wishlist count (read, instance check)'],
      favorites: ['POST   /api/users/:id/favorites               - Add favorite product (write, instance check)', 'DELETE /api/users/:id/favorites               - Remove favorite product (update, instance check)', 'POST   /api/users/:id/favorites/move-from-wishlist - Move from wishlist to favorites (update, instance check)'],
      cart: ['POST   /api/users/:id/cart                    - Add to cart (write, instance check)', 'DELETE /api/users/:id/cart                    - Remove from cart (update, instance check)', 'PUT    /api/users/:id/cart/quantity           - Update cart item quantity (update, instance check)', 'DELETE /api/users/:id/cart/clear              - Clear cart (update, instance check)', 'GET    /api/users/:id/cart/total              - Calculate cart total (read, instance check)', 'GET    /api/users/:id/cart/count              - Get cart item count (read, instance check)', 'POST   /api/users/:id/cart/move-to-wishlist   - Move from cart to wishlist (update, instance check)'],
      preferences: ['PUT    /api/users/:id/preferences             - Update preferences (update, instance check)', 'PUT    /api/users/:id/preferences/newsletter  - Toggle newsletter subscription (update, instance check)', 'PUT    /api/users/:id/preferences/notifications - Toggle notifications (update, instance check)', 'PUT    /api/users/:id/preferences/theme       - Set theme preference (update, instance check)', 'PUT    /api/users/:id/preferences/language    - Update language preference (update, instance check)'],
      loyalty: ['POST   /api/users/:id/loyalty/add             - Add loyalty points (update, instance check)', 'POST   /api/users/:id/loyalty/redeem          - Redeem loyalty points (update, instance check)', 'POST   /api/users/:id/loyalty/transfer        - Transfer loyalty points (update, instance check)', 'PUT    /api/users/:id/loyalty/reset           - Reset loyalty points (update, instance check)'],
      subscription: ['PUT    /api/users/:id/subscription            - Update subscription (update, instance check)', 'DELETE /api/users/:id/subscription            - Cancel subscription (update, instance check)'],
      accountStatus: ['PUT    /api/users/:id/status                  - Update status (update, instance check)', 'PUT    /api/users/:id/reactivate              - Reactivate account (update, instance check)', 'PUT    /api/users/:id/deactivate              - Deactivate account (update, instance check)', 'PUT    /api/users/:id/activate              - activate account (update, instance check)', 'PUT    /api/users/:id/lock                    - Lock account (update, instance check)', 'PUT    /api/users/:id/unlock                  - Unlock account (update, instance check)'],
      addresses: ['POST   /api/users/:id/addresses               - Add address (write, instance check)', 'DELETE /api/users/:id/addresses               - Remove address (update, instance check)', 'PUT    /api/users/:id/addresses/default       - Set default address (update, instance check)'],
      paymentMethods: ['POST   /api/users/:id/payment-methods         - Add payment method (write, instance check)', 'DELETE /api/users/:id/payment-methods         - Remove payment method (update, instance check)', 'PUT    /api/users/:id/payment-methods/default - Set default payment method (update, instance check)'],
      socialMedia: ['PUT    /api/users/:id/social-media            - Update social media (update, instance check)', 'POST   /api/users/:id/social-media/link       - Link social account (write, instance check)', 'DELETE /api/users/:id/social-media/unlink     - Unlink social account (update, instance check)', 'DELETE /api/users/:id/social-media/clear      - Clear all social links (update, instance check)'],
      interests: ['POST   /api/users/:id/interests               - Add interest (write, instance check)', 'DELETE /api/users/:id/interests               - Remove interest (update, instance check)', 'POST   /api/users/:id/interests/category      - Add interest category (write, instance check)', 'DELETE /api/users/:id/interests/clear         - Clear interests (update, instance check)'],
      sessions: ['POST   /api/users/:id/sessions/invalidate-all - Invalidate all sessions (update, instance check, rate-limited)', 'POST   /api/users/:id/sessions/revoke-token   - Revoke token (update, instance check)', 'PUT    /api/users/:id/login-timestamp         - Update login timestamp (update, instance check)', 'PUT    /api/users/:id/failed-logins/increment - Increment failed logins (update)', 'PUT    /api/users/:id/failed-logins/reset     - Reset failed logins (update, instance check)'],
      orders: ['POST   /api/users/:id/orders                  - Add order (write, instance check)', 'GET    /api/users/:id/orders                  - Get order history (read, instance check)'],
      reporting: ['GET    /api/users/:id/statistics              - Get user statistics (view, instance check)', 'GET    /api/users/:id/report                  - Get user report (report, instance check)', 'GET    /api/users/:id/activity-summary        - Get activity summary (report, instance check)', 'PUT    /api/users/:id/dynamic-update          - Dynamic update (update, instance check)'],
      search: ['GET    /api/users/search/email                - Find by email (read)', 'GET    /api/users/search/username             - Find by username (read)', 'GET    /api/users/search                      - Search users (read)', 'GET    /api/users/search/email-username       - Search by email or username (read)', 'GET    /api/users/search/dynamic              - Dynamic search (read)'],
      filter: ['GET    /api/users/filter/status               - Get users by status (read)', 'GET    /api/users/filter/active               - Get active users (read)', 'GET    /api/users/filter/verified             - Get verified users (read)', 'GET    /api/users/filter/role                 - Get users by role (read)', 'GET    /api/users/filter/admins               - Get admins (read)', 'GET    /api/users/filter/customers            - Get customers (read)', 'GET    /api/users/filter/subscription         - Get users by subscription type (read)', 'GET    /api/users/filter/active-within-days   - Find active users within days (read)', 'GET    /api/users/filter/top-loyal            - Get top loyal users (read)', 'GET    /api/users/filter/never-logged-in      - Get never logged in users (read)', 'GET    /api/users/filter/oldest               - Find oldest user (read)', 'GET    /api/users/filter/failed-logins        - Find users with failed logins (read)', 'GET    /api/users/filter/incomplete-profiles  - Find users with incomplete profiles (read)'],
      bulk: ['PUT    /api/users/bulk/update-role            - Bulk update role (update, rate-limited)', 'DELETE /api/users/bulk/delete                 - Bulk delete (update, rate-limited)', 'PUT    /api/users/bulk/update-status          - Bulk update status (update, rate-limited)', 'POST   /api/users/bulk/add-loyalty-points     - Bulk add loyalty points (update, rate-limited)'],
      analytics: ['GET    /api/users/analytics/count-by-role     - Get user count by role (view)', 'GET    /api/users/analytics/count-by-subscription - Get user count by subscription (view)', 'GET    /api/users/analytics/count-by-country  - Get user count by country (view)', 'GET    /api/users/analytics/average-loyalty-points - Get average loyalty points (view)', 'GET    /api/users/analytics/average-orders    - Get average orders per user (view)', 'GET    /api/users/analytics/loyalty-brackets  - Get user loyalty brackets (view)', 'GET    /api/users/analytics/top-interests     - Get top user interests (view)', 'GET    /api/users/analytics/registrations-over-time - Get registrations over time (view)', 'GET    /api/users/analytics/login-activity-over-time - Get login activity over time (view)', 'GET    /api/users/analytics/table-statistics   - Get table statistics (view)', 'GET    /api/users/analytics/table-report      - Get table report (report)', 'GET    /api/users/analytics/user-report/:userId - Get user report by ID (report, instance check)', 'GET    /api/users/analytics/activity-summary/:userId - Get activity summary by ID (report, instance check)', 'GET    /api/users/analytics/users-with-analytics - Get users with analytics (view)', 'GET    /api/users/analytics/user-engagement   - Get user engagement metrics (view)'],
      exportImport: ['GET    /api/users/export/data                 - Export users data (view)', 'GET    /api/users/export/statistics           - Export user statistics (view)', 'GET    /api/users/export/csv                  - Export users as CSV (view)', 'POST   /api/users/import/data                 - Import users data (write, rate-limited)', 'POST   /api/users/import/csv                  - Import users from CSV (write, rate-limited)'],
      enhanced: ['GET    /api/users/advanced-search             - Advanced search (read)', 'POST   /api/users/:id/notify                  - Send notification to user (update, instance check)'],
      documentation: ['GET    /api/users/docs/routes                 - Get API route documentation (view, dev-only)'],
    };
