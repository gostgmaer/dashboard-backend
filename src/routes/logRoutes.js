...
router.get('/', async (req, res) => {
  try {
    ...
    const logs = await Log.find(query).select('timestamp user_id user_fullname user_metadata action operation endpoint method response_status').sort({ timestamp: -1 }).limit(limit).lean();
...
// GET /logs/user - Get all logs related to the currently authenticated user
router.get('/user', async (req, res) => {
  try {
    const user = req.user; // Comes from authMiddleware

    const { limit = 10, page = 1, operation, action, start_date, end_date } = req.query;
    ...
    // Fetch logs
    const [logs, total] = await Promise.all([Log.find(query).select('timestamp action operation endpoint method response_status').sort({ timestamp: -1 }).skip(skip).limit(parseInt(limit)).lean(), Log.countDocuments(query)]);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        count: logsWithMessage.length,
        results: logsWithMessage,
      },
    });
  } catch (err) {
    console.error('Error fetching user logs:', err);
    res.status(500).json({ success: false, error: 'Failed to fetch user logs' });
  }
});
...
router.get('/activity/:table/:record_id', async (req, res) => {
  try {
    ...
    if (start_date && end_date) {
      logs = logs.filter((log) => new Date(log.timestamp) >= new Date(start_date) && new Date(log.timestamp) <= new Date(end_date));
    }
