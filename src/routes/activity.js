...
router.get('/my-activities', authMiddleware, async (req, res) => {
  try {
    const { limit = 50, days = 30, category, priority } = req.query;
...
router.get('/analytics', authMiddleware, async (req, res) => {
  try {
    ...
    const analytics = await ActivityHelper.getAnalytics(
      userId || null,
      parseInt(days)
    );
...
// Get popular routes
router.get('/popular-routes', authMiddleware, async (req, res) => {
  try {
    const { days = 7, limit = 10 } = req.query;

    const routes = await activityLogService.getPopularRoutes(
      parseInt(days),
      parseInt(limit)
    );
...
router.get('/export/:userId?', authMiddleware, async (req, res) => {
  try {
    ...
    if (format === 'csv') {
      const csv = [
        'Timestamp,User,Email,Action,Route,Method,IP,Status,Response Time',
        ...activities.map(activity => [
          activity.timestamp.toISOString(),
          activity.userId?.username || 'Unknown',
          activity.userId?.email || 'Unknown',
          activity.action,
          activity.route,
          activity.method,
          activity.ip,
          activity.statusCode || 'N/A',
          activity.responseTime || 'N/A'
        ].join(','))
      ].join('\n');
