...
import { sendSuccess, HTTP_STATUS } from '../../utils/responseHelper.js';
...
export const getDashboardData = catchAsync(async (req, res) => {
  ...
  const usersByDevice = await User.aggregate([{ $group: { _id: '$deviceType', count: { $sum: 1 } } }]);
  ...
  const revenueAgg = await Order.aggregate([{ $match: { status: 'completed' } }, { $group: { _id: null, totalRevenue: { $sum: '$total' }, avgOrderValue: { $avg: '$total' } } }]);
  const totalRevenue = revenueAgg[0]?.totalRevenue || 0;
  const avgOrderValue = revenueAgg[0]?.avgOrderValue || 0;

  const repeatPurchaseRate = 22.5;
  const cartAbandonmentRate = 18.7;
  ...
  const ordersByShipping = await Order.aggregate([{ $group: { _id: '$shippingMethod', count: { $sum: 1 } } }]);
  const paymentMethods = await Order.aggregate([{ $group: { _id: '$paymentMethod', count: { $sum: 1 }, revenue: { $sum: '$total' } } }]);
  const monthlyRevenue = await Order.aggregate([{ $match: { status: 'completed' } }, { $group: { _id: { $dateToString: { format: '%Y-%m', date: '$createdAt' } }, revenue: { $sum: '$total' } } }, { $sort: { _id: 1 } }]);

  const categoryRevenue = await Order.aggregate([{ $unwind: '$products' }, { $lookup: { from: 'products', localField: 'products.product', foreignField: '_id', as: 'product' } }, { $unwind: '$product' }, { $group: { _id: '$product.categories', revenue: { $sum: '$products.price' } } }]);

  const brandRevenue = await Order.aggregate([{ $unwind: '$products' }, { $lookup: { from: 'products', localField: 'products.product', foreignField: '_id', as: 'product' } }, { $unwind: '$product' }, { $group: { _id: '$product.brand', revenue: { $sum: '$products.price' } } }]);
  ...
  const userLocation = await User.aggregate([{ $group: { _id: '$country', users: { $sum: 1 } } }]);
  const dailyActiveUsers = await User.aggregate([{ $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$lastLogin' } }, count: { $sum: 1 } } }, { $sort: { _id: 1 } }]);
  ...
  const topRegionsByRevenue = await Order.aggregate([{ $match: { status: 'completed' } }, { $group: { _id: '$shippingAddress.region', revenue: { $sum: '$total' } } }, { $sort: { revenue: -1 } }, { $limit: 5 }]);
  ...
  const categoryInventory = await Product.aggregate([{ $group: { _id: '$categories', totalStock: { $sum: '$stock' }, inventoryValue: { $sum: '$price' } } }, { $project: { categoryName: '$_id', totalStock: 1, inventoryValue: 1, _id: 0 } }]);
  ...
  const highReturningCustomerOrders = await Order.find({ status: 'completed' }).limit(10);
  ...
  const inventoryOverview = {
    ...
    stockDistribution,
    inventoryTurnover: [{ product: 'Smartphone X', turnover: 3.5 }],
    backorderAlerts: [{ product: 'Laptop Pro', qty: 5 }],
  };
  ...
  return sendSuccess(res, { data: response, message: 'Dashboard data retrieved successfully' });
});
