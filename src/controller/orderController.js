...
function buildSafeProjection(fields) {
  // If UI does NOT send fields → use defaults
  const requested = fields
    ? fields.split(",").map(f => f.trim())
    : DEFAULT_ORDER_FIELDS;

  const safeFields = requested.filter(
    (f) => !SENSITIVE_FIELDS.some(s => f === s || f.startsWith(`${s}.`))
  );
...
class OrderController {
  ...
  async getOrders(req, res) {
    try {
      let { page = 1, limit = 20, sort = 'createdAt', order = 'desc', search,fields, ...rawFilters } = req.query;
    ...
    ========================= */
      const multiValueFields = ['status', 'payment_status', 'orderSource', 'priorityLevel', 'payment_method', 'currency', 'complianceStatus', 'shippingMethod'];

      multiValueFields.forEach((field) => {
        if (rawFilters[field]) {
          const values = rawFilters[field]
            .split(',')
            .map((v) => v.trim())
            .filter(Boolean);
    ...
    ========================= */
      if (rawFilters.user) {
        const users = rawFilters.user.split(',').filter((id) => this.isValidObjectId(id));

        if (users.length) {
          filter.user = { $in: users };
          appliedFilters.user = users;
        }
    ...
    ========================= */
      if (rawFilters.dateFrom || rawFilters.dateTo) {
        filter.createdAt = {};
        if (rawFilters.dateFrom) filter.createdAt.$gte = new Date(rawFilters.dateFrom);
        if (rawFilters.dateTo) filter.createdAt.$lte = new Date(rawFilters.dateTo);

        appliedFilters.createdAt = filter.createdAt;
        delete rawFilters.dateFrom;
        delete rawFilters.dateTo;
      }

      /* =========================
       AMOUNT RANGE FILTERS
    ========================= */
      ['total', 'amount_paid', 'amount_due'].forEach((field) => {
        const minKey = `${field}Min`;
        const maxKey = `${field}Max`;

        if (rawFilters[minKey] || rawFilters[maxKey]) {
          filter[field] = {};
          if (rawFilters[minKey]) filter[field].$gte = Number(rawFilters[minKey]);
          if (rawFilters[maxKey]) filter[field].$lte = Number(rawFilters[maxKey]);

          appliedFilters[field] = filter[field];
          delete rawFilters[minKey];
          delete rawFilters[maxKey];
        }
      });
    ...
    ========================= */
      if (search && search.trim()) {
        const q = search.trim();
        filter.$or = [{ email: { $regex: q, $options: 'i' } }, { firstName: { $regex: q, $options: 'i' } }, { lastName: { $regex: q, $options: 'i' } }, { phone: { $regex: q, $options: 'i' } }, { invoice: { $regex: q, $options: 'i' } }, { order_id: { $regex: q, $options: 'i' } }, { transaction_id: { $regex: q, $options: 'i' } }, { trackingNumber: { $regex: q, $options: 'i' } }];
      }

      /* =========================
       EXACT MATCH FILTERS
    ========================= */
      const exactMatchFields = ['entity', 'ipAddress', 'deviceInfo', 'carrier', 'fraudScore', 'created_by', 'updated_by'];

      exactMatchFields.forEach((field) => {
        if (rawFilters[field]) {
          filter[field] = rawFilters[field];
          appliedFilters[field] = rawFilters[field];
          delete rawFilters[field];
        }
      });
  const projection = buildSafeProjection(fields);
    ...
    ========================= */
      ...
      return standardResponse(res, true, response, `Retrieved ${result.length} orders`);
    } catch (error) {
      return standardResponse(res, false, null, error.message || 'Failed to fetch orders', 400);
    }
  }
