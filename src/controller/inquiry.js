...
class InquiryController {
  ...
  // SEND PROPOSAL - Send proposal PDF via email
  static sendProposal = catchAsync(async (req, res) => {
    const { proposalId, pdfUrl, recipientEmail, recipientName } = req.body;

    if (!proposalId || !pdfUrl || !recipientEmail) {
      throw AppError.badRequest('Missing required fields: proposalId, pdfUrl, recipientEmail');
    }

    // Find the inquiry
    const inquiry = await Inquiry.findById(proposalId);
    if (!inquiry) {
      throw AppError.notFound('Proposal/Inquiry not found');
    }

    // Format inquiry number for display (PROP-YYYYMM-XXXX)
    const now = new Date();
    const year = now.getFullYear();
    const month = (now.getMonth() + 1).toString().padStart(2, '0');
    const formattedNumber = `PROP-${year}${month}-${inquiry.inquiryNumber.toString().padStart(4, '0')}`;

    // Calculate valid until date (30 days from now)
    const validUntilDate = new Date();
    validUntilDate.setDate(validUntilDate.getDate() + 30);

    // Send email with proposal PDF using PROJECT_PROPOSAL_EMAIL template
    const emailData = {
      email: recipientEmail,
      clientName: recipientName || inquiry.name,
      projectName: inquiry.projectType || inquiry.company || 'Your Project',
      proposalUrl: pdfUrl,
      proposalNumber: formattedNumber,
      issueDate: now.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }),
      validUntil: validUntilDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }),
      companyName: 'EasyDEV',
      contactEmail: 'info@easydev.in',
    };

    await sendEmail(PROJECT_PROPOSAL_EMAIL, emailData);

    // Update inquiry status to indicate proposal was sent
    inquiry.status = 'quoted';
    inquiry.proposalSentAt = new Date();
    inquiry.proposalUrl = pdfUrl;
    await inquiry.save();
    ...
    // 	details: { recipientEmail, pdfUrl },
    // });

    return sendSuccess(res, {
      data: { sent: true, inquiry: inquiry.toAPIResponse() },
