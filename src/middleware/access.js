...
class authAccess {
    ...
    static async authenticate(req, res, next) {
        try {
            ...
            const tokenData = user.authTokens.find(t => 
                t.token === token && 
                !t.isRevoked && 
                t.type === 'access' &&
    ...
    static requireOTP(purpose = 'sensitive_op') {
        return async (req, res, next) => {
            try {
                ...
                const recentOTPVerification = req.user.currentOTP?.verified && 
                    req.user.currentOTP?.purpose === purpose &&
                    (Date.now() - new Date(req.user.currentOTP.verifiedAt).getTime()) < otpVerificationWindow;
    ...
    static requireTrustedDevice() {
        return (req, res, next) => {
            ...
            const device = req.user.knownDevices.find(d => 
                d.deviceId === req.deviceInfo.deviceId
            );
    ...
    static async performSecurityChecks(user, deviceInfo, tokenData) {
        ...
        if (deviceInfo.security?.riskLevel === 'high') {
            ...
            await user.logSecurityEvent('suspicious_device_blocked', 
                'High-risk device blocked', 'high', deviceInfo);
            return checks;
        }
        ...
        if (process.env.REQUIRE_DEVICE_VERIFICATION === 'true') {
            ...
            if (expectedDeviceId !== deviceInfo.deviceId) {
                ...
                await user.logSecurityEvent('device_mismatch', 
                    'Token used from different device', 'high', deviceInfo);
                return checks;
            }
        }
        ...
        if (process.env.ENABLE_IP_WHITELIST === 'true') {
            ...
            if (!ipAllowed) {
                ...
                await user.logSecurityEvent('ip_blocked', 
                    'IP not in whitelist', 'high', deviceInfo);
                return checks;
            }
        }
    ...
    static async checkOTPRequirement(req, user, deviceInfo) {
        ...
        if (result.required) {
            const otpVerificationWindow = 10 * 60 * 1000; // 10 minutes
            result.satisfied = user.currentOTP?.verified && 
                (Date.now() - new Date(user.currentOTP.verifiedAt || 0).getTime()) < otpVerificationWindow;
        }
    ...
    static async detectSuspiciousActivity(user, deviceInfo) {
        ...
        if (lastLogin && lastLogin.successful) {
            ...
            if (hoursDiff < 2) {
                const lastDevice = user.knownDevices.find(d => 
                    d.ipAddress === lastLogin.ipAddress
                );
                
                if (lastDevice && 
                    lastDevice.location?.country !== deviceInfo.location?.country &&
        ...
        const recentFailures = user.loginHistory?.filter(h => 
            !h.successful && 
            (Date.now() - new Date(h.loginTime).getTime()) < (24 * 60 * 60 * 1000) // 24 hours
        ).length || 0;
        ...
        const recentLogins = user.loginHistory?.filter(h => 
            h.successful && 
            (Date.now() - new Date(h.loginTime).getTime()) < (7 * 24 * 60 * 60 * 1000) // 7 days
        ) || [];

        const uniqueIPs = new Set(recentLogins.map(l => l.ipAddress));
        if (uniqueIPs.size > 10) { // More than 10 different IPs in a week
            result.score += 20;
